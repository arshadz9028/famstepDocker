...
import { registerMoreInfoValidate } from "../lib/validate";
import { getSession } from "next-auth/react";
import HelpModal from "../layouts/gettingStarted/HelpMessage";
import LocationModal from "../layouts/gettingStarted/LocationModal";
import SkillsModal from "../layouts/gettingStarted/SkillsModal";
import DesignationModel from "../layouts/gettingStarted/DesignationModel";
import InterestModal from "../layouts/gettingStarted/InterestModel";
import { ContextProvider } from "../global/context";
import axios from "axios";
import { useRouter } from "next/router";
import User from "../model/userModel";
function SignupMoreDetail({ session }) {
  const {
    selectedLocation,
    selectedSkills,
    selectedInterests,
  ...
  const [interestModel, setInterestModel] = useState(false);
  const [interestModelAtleast, setInterestModelAtleast] = useState(false);
  const [locationModal, setLocationModal] = useState(false);
  const router = useRouter();
  //handle location & skills function
  const LocationTab = (e) => {
    if (true || e.key === "Tab") {
  ...
  const InterestTab = (e) => {
    if (true || e.key === "Tab") {
      setInterestModel(!interestModel);
      setInterestModelAtleast(false)
    }
  };
  const DesignationTab = (e) => {
    if (true || e.key === "Tab") {
  ...
  const formik = useFormik({
    ...
    validate: registerMoreInfoValidate,
    onSubmit,
  });

  useEffect(() => {
    const combine = [selectedLocation.city, selectedLocation.country]
      .filter(Boolean)
      .join();
    formik.setValues((prevValues) => ({
      ...prevValues,
      designation: selectedDesignation,
      locationUser: combine,
      userHobbies: selectedInterests,
  ...
  useEffect(() => {
    ...
    if (help || locationModal || skillModal) {
      document.body.style.overflow = "hidden";
    } else {
      document.body.style.overflow = "unset";
      document.body.style.overflowX = "hidden";
    }
  }, [help, locationModal, skillModal]);

  async function onSubmit(values) {
    try {
      const response = await axios.post("/api/auth/signup/gettingStarted", {
        values,
      });
  ...
  const skipFunction = async () => {
    setInterestModel(true)
    setInterestModelAtleast(true)
  ...
  return (
    <>
      ...
      {help && <HelpModal setHelp={setHelp} />}
      {skillModal && (
        <SkillsModal setSkillModal={setSkillModal} Value={"Next"} />
      )}
      {designationModel && (
        <DesignationModel setDesignationModel={setDesignationModel} />
      )}
      {interestModel && <InterestModal setInterestModel={setInterestModel} interestModelAtleast={interestModelAtleast} />}
          ...
          <div className={styles.signup_form}>
            <div>
              <div className={styles.login_title}>
                ...
                Share a few details to help us personalize your experience!
                </span>
              </div>
              <form onSubmit={formik.handleSubmit} autoComplete="off">
                <div
                  className={styles.input_bx}
                  onClick={DesignationTab}
                  onKeyDown={DesignationTab}
                >
                  <input
                    type="text"
                    id="designation"
                ...
                >
                  <input
                    ...
                    name="userHobbies"
                    required="required"
                    {...formik.getFieldProps("userHobbies")}
                    style={
                      formik.errors.userHobbies && formik.touched.userHobbies
                        ? { outline: "1px solid red" }
                        : null
                    }
                  />
                ...
                >
                  <input
                    ...
                    name="userSkills"
                    required="required"
                    {...formik.getFieldProps("userSkills")}
                    style={
                      formik.errors.userSkills && formik.touched.userSkills
                        ? { outline: "1px solid red" }
                        : null
                    }
                  />
                ...
                >
                  <input
                    ...
                    name="locationUser"
                    required="required"
                    {...formik.getFieldProps("locationUser")}
                    style={
                      formik.errors.locationUser && formik.touched.locationUser
                        ? { outline: "1px solid red" }
                        : null
                    }
                  />
              ...
              <input
                type="button"
                styles={{ outline: "none", cursor:"pointer" }}
...
export async function getServerSideProps({ req }) {
  ...
  } else if (!session.user.userEmailVerified) {
    return {
      redirect: {
        destination: `/emailVerify?user=${session.user.username}`,
        permanent: false,
      },
    };
...
export default SignupMoreDetail;
