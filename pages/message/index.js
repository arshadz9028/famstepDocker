...
function Message({ session, userMsgData }) {
    ...
    <>
      ...
      <div className={styles.message_main}>
        <div className={styles.message_main_center}>
                ...
                MessgeFriends.slice() // Create a copy of the array to avoid mutating the original array
                  ...
                  .map(
                    (val) =>
                      true && (
                        <Messagefriend
                          key={val._id} // Assign the key directly to the Messagefriend component
                          val={val}
          ...
          {!newmessage ? (
            <div className={styles.messaging_section1}>
              ...
              <p className={styles.message_line}>
                Engage in private conversations with your friends on Famstep{" "}
              </p>
              <input
    ...
    let userMsgData = await Chat.find({
      ...
      userMsgData.forEach((value, index) => {
        if (value.latestMessage) {
          const decryptedTexts = CryptoJS.AES.decrypt(
            value.latestMessage.content,
            process.env.ENCRYPTION_KEY
          ).toString(CryptoJS.enc.Utf8);
          userMsgData[index].latestMessage.content = decryptedTexts;
        }
      });
