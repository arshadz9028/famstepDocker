...
function UserReport({ setdialogeReport, messageId ,otherUserId }) {
  ...
  const handleReport = async (reportReason) => {
    ...
    try {
      ...
      await axios.post(
        `/api/report/profileReport`,
        {  reason: reportReason, userId: otherUserId }
      );
    }else{
      await axios.post(
        `/api/report/profileReport`,
        {  reason: reportReason, userId: profile }
      );
    }
    } catch (error) {}
